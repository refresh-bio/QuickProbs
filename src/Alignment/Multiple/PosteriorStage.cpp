	...
	Array<SparseMatrixType*>& sparseMatrices)
{
	...
	LOG_DEBUG << "Max sequence length = " << maxSeqLength << endl
		<< "Max layer size = " << maxLayerSize << endl;
	...
	STATS_WRITE("memory.posterior", denseBytes);
	STATS_WRITE("memory.sparse", sparseBytes);
}
